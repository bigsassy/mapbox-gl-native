...
public:
    ...
    OffscreenView view { context.getGLContext(), { 512, 512 } };
...
TEST(Memory, Vector) {
    ...
    Map map(test.context, { 256, 256 }, 2, test.fileSource, test.threadPool, MapMode::Still);
...
TEST(Memory, Footprint) {
    ...
    for (unsigned i = 0; i < runs; ++i) {
        auto vector = std::make_unique<Map>(test.context, mbgl::Size{ 256, 256 }, 2, test.fileSource,
    ...
    for (unsigned i = 0; i < runs; ++i) {
        auto raster = std::make_unique<Map>(test.context, mbgl::Size{ 256, 256 }, 2, test.fileSource,
                                            test.threadPool, MapMode::Still);
